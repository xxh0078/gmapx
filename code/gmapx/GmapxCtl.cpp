...
BEGIN_DISPATCH_MAP(CGmapxCtrl, COleControl)
	...
	DISP_FUNCTION(CGmapxCtrl, "SetMapToolVisible", SetMapToolVisible, VT_I4, VTS_I4 VTS_I4)
	DISP_FUNCTION(CGmapxCtrl, "SetMarkFile", SetMarkFile, VT_I4, VTS_BSTR)
	DISP_FUNCTION(CGmapxCtrl, "GetScreenXY", GetScreenXY, VT_BSTR, VTS_R8 VTS_R8)
	DISP_FUNCTION(CGmapxCtrl, "SetMapLineVisible", SetMapLineVisible, VT_I4, VTS_I4 VTS_I4)
	DISP_FUNCTION(CGmapxCtrl, "SetMapPointAngle", SetMapPointAngle, VT_I4, VTS_I4 VTS_I4)
	//}}AFX_DISPATCH_MAP
...
long CGmapxCtrl::SetMapToolVisible(long lType, long lVisible) 
{
	// TODO: Add your dispatch handler code here
	if( lType == 1 )
	{
		if ( m_pMapEngine )
		{
			m_pMapEngine->m_MapCtrl.SetVisible( lVisible );
		}
	}
	if( lType == 2 )
	{
		if ( m_pMapEngine )
		{
			m_pMapEngine->m_MapCtrl.SetVisible1( lVisible );
		}
	}
	if( lType == 3 )
	{
		g_bRightKey = lVisible;
	}
	return 0;
}

long CGmapxCtrl::SetMarkFile(LPCTSTR strFileName) 
{
	// TODO: Add your dispatch handler code here
	if ( m_pMapEngine )
	{
		m_pMapEngine->SetMarkFile( strFileName );
	}
	return 0;
}

BSTR CGmapxCtrl::GetScreenXY(double dMapX, double dMapY) 
{
	CString strResult;
	// TODO: Add your dispatch handler code here
	if ( m_pMapEngine )
	{
		long lx=0;
		long ly=0;
		m_pMapEngine->MapToScreen( dMapX, dMapY, lx,ly );
		strResult.Format("%d,%d",lx,ly);
	}
	return strResult.AllocSysString();
}

long CGmapxCtrl::SetMapLineVisible(long rid, long lVisible) 
{
	// TODO: Add your dispatch handler code here
	if ( m_pMapEngine )
	{
		return  m_pMapEngine->m_MapDisp.m_MapLayerMan.SetMapLineVisible( rid, lVisible );
	}
	return 0;
}

long CGmapxCtrl::SetMapPointAngle(long pid, long lAngle) 
{
	// TODO: Add your dispatch handler code here
	if ( m_pMapEngine )
	{
		return m_pMapEngine->m_MapDisp.m_MapLayerMan.SetMapPointAngle( pid, lAngle );
	}
	return 0;
}
