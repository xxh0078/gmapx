...
NOTES:			Provides an interface to the CMD5 class. 'strFilePath' name should 
				...
				exception.
*****************************************************************************************/
//CString CMD5::GetMD5(const CString& strFilePath)
//{
...
NOTES:			Provides an interface to the CMD5 class. 'File' should be open in 
...
*****************************************************************************************/
//CString CMD5::GetMD5(CFile& File)
//{
//	try
//	{
//		CMD5 MD5Checksum;		//checksum object	
...
//	{
//		TRACE0("CMD5::GetMD5: CFileException caught");	
//		throw e;
//	}
//}
...
NOTES:			Provides an interface to the CMD5 class. Any data that can
...
*****************************************************************************************/
char* CMD5::GetMD5(BYTE* pBuf, UINT nLength)
{
	//entry invariants
...
DETAILS:		static, public
DESCRIPTION:	Gets the MD5 checksum for data in a BYTE array
RETURNS:		unsigned char : the hexadecimal MD5 checksum for the specified data
ARGUMENTS:		BYTE* pSourceBuf  :	pointer to the BYTE array
				UINT nSourceLength : number of BYTEs of data to be checksumed
				pResultBuf : buffer for result
...
NOTES:			Provides an interface to the CMD5 class. Any data that can
				...
				will return the same checksum. 
****************************************************************************************/
unsigned char * CMD5::GetMD5( unsigned char* pSourceBuf, unsigned int nSourceLength, unsigned char pResultBuf[ 32 ] )
{
	//calculate and return the checksum
	CMD5 MD5;
	MD5.Update( pSourceBuf, nSourceLength );
	return MD5.Final( pResultBuf );
}

/*****************************************************************************************
FUNCTION:		CMD5::RotateLeft
DETAILS:		private
...
DETAILS:		protected
DESCRIPTION:	Implementation of main MD5 checksum algorithm; ends the checksum calculation.
RETURNS:		unsigned char * : the final hexadecimal MD5 checksum result 
ARGUMENTS:		None
NOTES:			Performs the final MD5 checksum calculation ('Update' does most of the work,
				this function just finishes the calculation.) 
*****************************************************************************************/
unsigned char * CMD5::Final( unsigned char pResultBuf[ 32 ] )
{
	//Save number of bits
	...
	//Convert the hexadecimal checksum to a unsigned char *
	unsigned char strMD5[ 64 ];
	memset( (char *)strMD5, 0, 64 );
	for ( int i=0; i < nMD5Size; i++) 
	{
		unsigned char Str[ 8 ];
		if ( lpszMD5[i] == 0 ) 
		{
			strcpy( (char *)Str, ("00") );
//			Str = unsigned char *("00");
		}
		else if (lpszMD5[i] <= 15) 	
		{
			sprintf( (char*)Str, "0%x", lpszMD5[i] );
//			Str.Format( _T("0%x"),lpszMD5[i]);
		}
		else 
		{
			sprintf( (char*)Str, "%x", lpszMD5[i] );
//			Str.Format( _T("%x"),lpszMD5[i]);
		}

		assert( strlen( (char*)Str ) == 2 );
		strcat( (char*)strMD5, (char*)Str );
	}
	assert( strlen( (char*)strMD5 ) == 32 );
	memcpy( pResultBuf, strMD5, 32 );
	return pResultBuf;
}
